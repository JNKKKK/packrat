"""Shared pre-flight guards for the review/merge job handlers.

dedup, cleanup, and merge all (a) resolve their target root and reject a non-library
one, and (b) refuse to run while *another* op holds that root. The dequeue gate
...
def reject_if_held(ctx, root: dict, *, ignore_merge: bool = False) -> None:
    """Raise ``ValueError`` if *another* active op holds ``root`` (per-root lock).
    ...
    by its own row. The dequeue gate covers the owning job; this re-checks for
