r"""The ``trash refresh`` job — absorb + empty the registered trash roots.
...
**Idempotent by construction** (record-then-delete), so it needs no special
...
the global single-worker slot. There is **no ``--dry-run``**: refresh is never a
no-op.
"""
...
def _run_trash_refresh(ctx: JobContext) -> None:
    ...
    # / TUI mascot modal); absent → every trash root (the cleanup/merge default).
...
register_job(
    JobSpec(
        ...
        owned_root=None,  # targets all trash roots; owns no library root
    )
)
