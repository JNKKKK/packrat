...
roots(
  ...
  last_probe_at   /* when `probe` last completed on this root (see workflow-scan.md; recency display) */,
...
assets(
  ...
  undecodable /* 0|1, default 0: bytes hashed OK but the decoder rejected the pixels — set by scan;
                 ...
                 yet) — see the "fully fingerprinted" predicate in workflow-scan.md A2 step 4. */,
  decode_error /* nullable text: last decoder failure detail, for debugging POC-format wheels (tech-stack.md) */,
  codec /* nullable text, VIDEO only: codec name (h264|hevc|av1|vp9|…) from the decode probe (workflow-scan.md
           step 8). Feeds the video stage-2 keep-lead's codec-efficiency weight (workflow-dedup.md match.codec_weights).
           NULL for photo/undecodable. NOT recomputed by `scan --full` (which skips re-decoding a
           byte-unchanged, fully-fingerprinted hit — workflow-scan.md A2 step 6), so a pre-existing NULL persists. */,
...
file_instances(   -- presence = row existence; a gone file has its row deleted (no 'present' flag)
  ...
  --   (workflow-scan.md A2 step 6), so re-encountering a known file (--full re-hash, mtime drift, merge-created
  --   backfill) reuses its row instead of creating a duplicate instance. `path` is stored in the
  --   canonical long-path-safe form (workflow-scan.md A1 step 1) so equality is well-defined.
...
similarity_edges(   -- pairwise near-dups; written by `dedup`, NOT scan. `distance` = PDQ Hamming
                    ...
                    --   review stages via t_photo_recompress (workflow-dedup.md); the edge itself is unbanded.
...
review_runs(   -- one stateful review lifecycle (dedup OR perceptual-cleanup) per target root
  ...
  stage_phase /* staged (shortcuts written, awaiting user) | applied (this stage's deletions done,
                 next stage not yet staged) — the apply-then-advance crash marker (workflow-dedup.md Phase 7) */,
  prefer_internal /* dedup --prefer-internal: keep the INTERNAL copy on exact-match survivor +
                      ...
                      A conflicting flag on a later --confirm is rejected (workflow-dedup.md). */,
  t_photo_recompress, t_photo_edit, t_match_video /* PDQ thresholds SNAPSHOTTED at analyze (workflow-dedup.md),
  ...
  --   `status` stays 'pending' until the LAST non-empty stage applies (workflow-dedup.md).
  -- One facet of the architecture.md per-root exclusivity invariant: dedup, perceptual-cleanup, in-flight
  --   merge, AND scan are mutually exclusive on a root (scan is blocked by workflow-scan.md A2 step 1a, not by
...
review_actions(   -- the persisted, crash-safe plan for a review_run
  ...
  shortcut_name )
  -- `path` is the AUTHORITATIVE target: --confirm re-stats it (workflow-dedup.md Phase 6) and never trusts the
...
merge_runs(   -- one merge lifecycle (workflow-merge.md); the frozen plan header + cross-op guard
  ...
  --     dedup (workflow-dedup.md Phase 0) and cleanup (trash-model.md) check to refuse an overlapping run.
  -- completed runs are retained (queryable merge history; see workflow-dedup.md note / roadmap.md #5).

merge_plan_items(   -- the persisted, crash-safe, FROZEN per-source-file plan for a merge_run
  id, run_id,
  source_rel_path,                 -- path relative to source; dest mirrors it (workflow-merge.md Phase 3)
  ...
  --                      (would otherwise be forgotten by the next scan's deletion-detection — workflow-merge.md Phase 3)
...
jobs(    id, type,
         ...
         result_json /* nullable: a compact, uniform, human-showable OUTCOME summary written at
                        ...
                        the workflow-dedup.md audit) stay authoritative for deep forensics ([Enter] details). A job
  ...
  --   (workflow-scan.md A2), merge from per-item merge_plan_items.progress (workflow-merge.md), review from review_actions
  --   (workflow-dedup.md). `done` may be stale after a crash (last increment uncommitted) and that's harmless,
...
scan_results(   -- persisted scan report; one row per (completed scan job, root) so `status <root>`
                --   (and the M6 TUI) can re-render a past scan (workflow-scan.md A2 Phase 5, cli.md).
  ...
  undecodable, errors, deleted_instances, forgotten_assets, root_offline,   -- the workflow-scan.md A2 banner counts
  ...
  --   a resume/incremental re-run FAST-PATH-SKIPS undecodables (they're "fully fingerprinted", workflow-scan.md A2
...
- **Unreachable-root / incomplete-listing guard (per-directory):** deletion-detection (removing
  ...
  one flaky folder doesn't stall reconciliation collection-wide (see [scan](workflow-scan.md) A2 step 11, [performance](performance.md)).
